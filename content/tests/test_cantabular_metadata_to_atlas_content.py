...
from cantabular_metadata_to_atlas_content import filter_content
...
def test_filter_content_filters_and_copies_required_objects():
    ...
    returned_variable_group = filter_content([test_spec_row], [test_variable_group])[0]

    # THEN we expect to get a variable group back that is NOT our original variable group...
...
def test_filter_content_aggregates_variables_from_multiple_spec_rows():
    ...
    # AND given two spec rows that each specify a classification from the same variable
    test_spec_rows = [
    ...
    # WHEN we invoke filter_content against our test values
    returned_variable_groups = filter_content(test_spec_rows, [test_variable_group])

    # THEN we expect to get a single returned variable group back
    assert len(returned_variable_groups) == 1

    # AND we expect our variable group to have a single variable, that is NOT the original target variable...
...
def test_filter_content_adds_2011_comparison_flags():
    # GIVEN a CensusVariableGroup that has two CensusVariables with three CensusClassifications
    test_variable_group = get_test_variable_group()
    ...
    returned_variable_groups = filter_content(test_spec_rows, [test_variable_group])
     ...
     # THEN we expect to get a single variable group and two variable back
    assert len(returned_variable_groups) == 1
    returned_variables = returned_variable_groups[0].variables
